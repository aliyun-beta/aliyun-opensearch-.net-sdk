...
namespace AliCloudOpenSearch.com.API
{
    ...
    public class CloudsearchDoc
    {
       ...
       */
        ...
        /// 根据doc id获取doc的详细信息。
        /// </summary>
        /// <param name="pkField">指定的字段名称</param>
        /// <param name="docId">指定的doc id</param>
        /// <returns></returns>
        public async Task<Response> DetailAsync(string pkField, string docId)
        {
            var parameters = new Dictionary<string, object>();
            parameters.Add(pkField, docId);
            var resp = await client.ApiCallAsync(path, parameters, "POST").ConfigureAwait(false);
        ...
        /// 操作docs。
        /// </summary>
        /// <param name="docs">此docs为用户push的数据，此字段为json_encode的字符串或者数据。</param>
        /// <param name="tableName">操作的表名</param>
        /// <returns>请求API并返回相应的结果。</returns>
        private async Task<Response> doActionAsync(List<Dictionary<string, object>> docs, string tableName)
        {
            var parameters = new Dictionary<string, object>();
            parameters.Add("action", "push");
            parameters.Add("items", JsonConvert.SerializeObject(docs));
            ...
            var rawRet = await client.ApiCallAsync(path, parameters, "POST").ConfigureAwait(false);
            return JsonConvert.DeserializeObject<Response>(rawRet);
        }
#endif
        ...
        /// </summary>
        /// <param name="tableName"></param>
        public async Task<Response> PushAsync(string tableName)
        {
            var ret = await doActionAsync(_batchExecuteCachedDocs, tableName).ConfigureAwait(false);

            if (ret.Status != "OK")
            {
                for (var i = 0; i < MaxTryTimes; i++)
                {
                    ret = await doActionAsync(_batchExecuteCachedDocs, tableName).ConfigureAwait(false);
                    if (ret.Status == "OK")
                    {
                        break;
                    }
        ...
        ///     1,Int32 maxSize = PUSH_MAX_SIZE, Int32 frequence = PUSH_FREQUENCE)
        /// </prototype>
        public async Task<Dictionary<string, string>> PushHADocFileAsync(string fileName, string tableName, int offset = 1,
            int maxSize = PUSH_MAX_SIZE, int frequence = PUSH_FREQUENCE)
        {
            var reader = _connect(fileName);
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (separator == HA_DOC_ITEM_SEPARATOR)
                {
                    ...
                    {
                        // push 数据到api。
                        var rel = await doActionAsync(buffer, tableName).ConfigureAwait(false);
            ...
            if (buffer.Count > 0)
            {
                ...
                var rel1 = await doActionAsync(buffer, tableName).ConfigureAwait(false);
